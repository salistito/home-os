...
<template>
  <section class="mx-auto max-w-3xl">
    <div class="mb-5 flex items-baseline gap-2">
      <h2 class="text-lg font-semibold text-slate-900">Tareas</h2>
      <span v-if="!loading && !error" class="text-sm text-slate-400">
        ({{ tasks.length }})
      </span>
    </div>

    <p v-if="loading" class="text-sm text-slate-400">Cargando tareas…</p>
    ...
    <p
      ...
      class="rounded-lg border border-dashed border-slate-300 px-4 py-10 text-center text-sm text-slate-500"
    >
    ...
    >
      <div
        class="grid grid-cols-[1fr_5rem_7rem_6rem] items-center gap-3 border-b border-slate-200 bg-slate-50/60 px-4 py-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400"
      >
        <span>Tarea</span>
        <span>Puntos</span>
