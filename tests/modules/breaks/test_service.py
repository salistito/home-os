...
def test_create_break_period_ok(mock_get_user, mock_repo):
    ...
    result = create_break_period("Vacaciones", "2026-03-10", "2026-03-20", [1], ["tasks"])
...
def test_create_break_period_invalid_start_date(mock_repo):
    result = create_break_period("Vacaciones", "not-a-date", None, [1], ["tasks"])
...
def test_create_break_period_invalid_end_date(mock_repo):
    result = create_break_period("Vacaciones", "2026-03-10", "10/03/2026", [1], ["tasks"])
...
def test_create_break_period_invalid_date_range(mock_repo):
    result = create_break_period("Vacaciones", "2026-03-20", "2026-03-10", [1], ["tasks"])
...
def test_create_break_period_invalid_user_ids_empty(mock_repo):
    result = create_break_period("Vacaciones", "2026-03-10", None, [], ["tasks"])
...
def test_create_break_period_invalid_user_ids_wrong_type(mock_repo):
    result = create_break_period("Vacaciones", "2026-03-10", None, [1, "a"], ["tasks"])
...
def test_create_break_period_invalid_user_ids_unknown(mock_get_user, mock_repo):
    ...
    result = create_break_period("Vacaciones", "2026-03-10", None, [9999], ["tasks"])
...
def test_create_break_period_blank_label_normalized(mock_get_user, mock_repo):
    ...
    result = create_break_period("   ", "2026-03-10", None, [1], ["tasks"])
...
@patch("modules.breaks.service.repository")
@patch("modules.breaks.service.get_active_user_by_id")
def test_create_break_period_invalid_modules_empty(mock_get_user, mock_repo):
    mock_get_user.return_value = User(1, "Test", "admin")

    result = create_break_period("Vacaciones", "2026-03-10", None, [1], [])

    assert result.status == BreakPeriodOperationStatus.INVALID_MODULES
    mock_repo.create_break_period.assert_not_called()
...
@patch("modules.breaks.service.repository")
@patch("modules.breaks.service.get_active_user_by_id")
def test_create_break_period_invalid_modules_unknown(mock_get_user, mock_repo):
    mock_get_user.return_value = User(1, "Test", "admin")

    result = create_break_period("Vacaciones", "2026-03-10", None, [1], ["tasks", "bogus"])
...
@patch("modules.breaks.service.repository")
@patch("modules.breaks.service.get_active_user_by_id")
def test_update_break_period_invalid_modules(mock_get_user, mock_repo):
    mock_repo.get_break_period_by_id.return_value = _make_break_period()
    mock_get_user.return_value = User(1, "Test", "admin")

    result = update_break_period(1, modules=["bogus"])

    assert result.status == BreakPeriodOperationStatus.INVALID_MODULES
    mock_repo.set_break_period_modules.assert_not_called()


@pytest.mark.unit
@patch("modules.breaks.service.repository")
@patch("modules.breaks.service.get_active_user_by_id")
def test_update_break_period_with_modules(mock_get_user, mock_repo):
    mock_get_user.return_value = User(1, "Test", "admin")
    mock_repo.get_break_period_by_id.side_effect = [
        _make_break_period(modules=["tasks"]),
        _make_break_period(modules=["food"]),
    ]

    result = update_break_period(1, modules=["food"])

    assert result.status == BreakPeriodOperationStatus.OK
    mock_repo.update_break_period.assert_not_called()
    mock_repo.set_break_period_modules.assert_called_once_with(1, ["food"])
...
def test_get_break_summary_by_user_inside_period(mock_repo):
    ...
    result = get_break_period_summary_by_user("2026-03", "tasks")
...
@pytest.mark.unit
@patch("modules.breaks.service.repository")
def test_get_break_summary_by_user_module_filter(mock_repo):
    mock_repo.get_break_periods.return_value = [
        _make_break_period(start_date="2026-03-10", end_date=None, modules=["food"])
    ]
...
def test_get_break_summary_by_user_invalid_month(mock_repo):
    result = get_break_period_summary_by_user("not-a-month", "tasks")
