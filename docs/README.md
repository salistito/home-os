...
- **Sets breakdown**: campo propio de la sesion (columna JSON, no va dentro de `metrics`) con filas de fuerza `{name?, weight_kg?, reps, sets?}` (max 50; peso opcional para trabajo en peso corporal). El volumen y las reps no se persisten: se derivan al serializar (`volume_kg = sumatoria(peso x reps x series)` sobre filas con carga, `null` si ninguna tiene; `total_reps = sumatoria(reps x series)`).
...
- **Stats**: ultimo peso, deltas vs hace 7/30 dias, minutos y sesiones de los ultimos 7/30 dias, volumen (`volume_kg_last_7d/30d`) y reps totales (`reps_last_7d/30d`), mas minutos por nombre de ejercicio en los ultimos 30 dias (`by_exercise_last_30d`, incluye ejercicios eliminados).
